...
module uart(
    ...
    output [7:0] rxd_data // Parallel output to FPGA
);
    
//Internal Variables
reg shift; //triggering the shifting of data.
reg state, next_state; //initial state abd next variable state
...
assign rxd_data = rxshift_reg [8:1]; //assigning the rxd_data from the shift register
...
begin 
    if (reset) begin //if reset button is pressed, all counters are rset
        ...
        if (baudrate_counter >= div_counter-1) begin //if the counter reach the BR with sampling
            ...
            state <= next_state; //it should be ready to receive the data/switch to receiving state
            if (shift)rxshift_reg <= {inp_Rxd,rxshift_reg[9:1]}; //if shift is asserted , then load the receiving data
...
case (state)
    0: begin //Idle State
        if (inp_Rxd) begin //if input inp_Rxd line is asserted
          next_state <=0; //stay in the idle state, inp_Rxd needs to be low to start the transmission
        end
        else begin 
            next_state <=1; //receiving state
    ...
    1: begin // Receiving State
        ...
        if (sample_counter== mid_sample - 1) shift <= 1; //if the sample counter is 1, trigger garbage
        ...
        else inc_samplecounter <=1; //if the sample counter is not 4, or not equal to 3 (0-3)
    end
    default: next_state <=0; //idle state
endcase
end         
endmodule
